...
function convertToVietnamDateString(mysqlDate) {
    let utcDate;
...
router.get("/weekly", authMiddleware, async (req, res) => {
    try {
        ...
        const [dbRows] = await db_1.default.query(`SELECT DATE(log_date) as log_date, COUNT(*) as count
       FROM habit_logs
       WHERE user_id = ? AND log_date >= ? AND log_date < ?
       GROUP BY DATE(log_date)
       ORDER BY DATE(log_date) ASC`, [req.user.id, startDate, nextDayStr]);
...
router.get("/categories", authMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query(`SELECT h.category, COUNT(hl.id) as completed
       FROM habits h
       LEFT JOIN habit_logs hl ON h.id = hl.habit_id
       WHERE h.user_id = ? AND h.is_active = 1
       GROUP BY h.category`, [req.user.id]);
        res.json({ data: rows });
    }
    catch (error) {
        console.log(error);
...
router.get("/summary", authMiddleware, async (req, res) => {
    try {
        const [totalRows] = await db_1.default.query(`SELECT COUNT(*) as total_checkins,
              COUNT(DISTINCT log_date) as active_days
       FROM habit_logs WHERE user_id = ?`, [req.user.id]);
        const [streakRows] = await db_1.default.query(`SELECT longest_streak FROM streaks WHERE user_id = ?`, [req.user.id]);
        const [habitRows] = await db_1.default.query(`SELECT COUNT(*) as total_habits FROM habits WHERE user_id = ? AND is_active = 1`, [req.user.id]);
        res.json({
            summary: {
...
router.get("/habits/:habitId/metrics", authMiddleware, async (req, res) => {
    try {
        const { habitId } = req.params;
        const { days = 30 } = req.query; // Mặc định 30 ngày
        // Lấy thông tin habit
        const [habitRows] = await db_1.default.query(`SELECT h.*, 
              (SELECT COUNT(*) FROM habit_logs WHERE habit_id = h.id) as total_logs
       FROM habits h
       WHERE h.id = ? AND h.user_id = ?`, [habitId, req.user.id]);
        ...
        const [dbRows] = await db_1.default.query(`SELECT log_date, metric_value, metric_unit, note
       FROM habit_logs
       WHERE habit_id = ? AND user_id = ? 
         AND log_date >= ? AND log_date < ?
       ORDER BY log_date ASC`, [habitId, req.user.id, startDateStr, nextDayStr]);
        ...
        const logsWithMetrics = metricsRows.filter((row) => row.metric_value !== null);
        const totalValue = logsWithMetrics.reduce((sum, row) => sum + (parseFloat(row.metric_value) || 0), 0);
...
router.get("/habits/overview", authMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query(`SELECT h.id, h.name, h.category, h.icon, h.current_streak, h.longest_streak,
