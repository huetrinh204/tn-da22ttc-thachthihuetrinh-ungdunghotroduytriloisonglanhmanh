...
const fcm_push_service_1 = require("../services/fcm_push_service");
...
router.get("/streak", authMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT * FROM streaks WHERE user_id = ?", [req.user.id]);
...
router.get("/", authMiddleware, async (req, res) => {
    try {
        const [habits] = await db_1.default.query("SELECT * FROM habits WHERE user_id = ? AND is_active = 1 ORDER BY created_at DESC", [req.user.id]);
        res.json({ habits });
    }
    catch (error) {
        console.log(error);
...
router.get("/today", authMiddleware, async (req, res) => {
    try {
        ...
        const [habits] = await db_1.default.query(`SELECT h.*, 
       ...
       ORDER BY h.created_at DESC`, [today, req.user.id]);
...
router.post("/", authMiddleware, async (req, res) => {
    ...
    if (!name)
        return res.status(400).json({ message: "Tên thói quen không được trống" });
    try {
        const finalReminderTime = (reminder_enabled !== false && reminder_time) ? reminder_time : null;
        const [result] = await db_1.default.query(`INSERT INTO habits (user_id, name, category, frequency, target_count, icon, color, reminder_time)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
            req.user.id,
            name,
            category || "other",
        ...
        const [rows] = await db_1.default.query("SELECT * FROM habits WHERE id = ?", [result.insertId]);
...
router.put("/:id", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        await db_1.default.query(`UPDATE habits SET name = ?, category = ?, icon = ?, color = ?, target_count = ?, reminder_time = ?
       WHERE id = ? AND user_id = ?`, [name, category, icon, color, target_count, finalReminderTime, req.params.id, req.user.id]);
        // Push ngay nếu reminder vừa set gần giờ hiện tại
        await sendImmediateIfNeeded(req.user.id, name, finalReminderTime);
...
async function sendImmediateIfNeeded(userId, habitName, reminderTime) {
    if (!reminderTime)
        return;
    try {
        const now = new Date();
        ...
        if (diff > 0 && diff <= 10) {
            const [userRows] = await db_1.default.query("SELECT fcm_token FROM users WHERE id = ?", [userId]);
            if (userRows.length > 0 && userRows[0].fcm_token) {
                await (0, fcm_push_service_1.sendPushNotification)(userRows[0].fcm_token, '⏰ Đến giờ rồi!', `Đã đến lúc thực hiện thói quen "${habitName}" 💪`, userId);
...
router.delete("/:id", authMiddleware, async (req, res) => {
    try {
        await db_1.default.query("UPDATE habits SET is_active = 0 WHERE id = ? AND user_id = ?", [req.params.id, req.user.id]);
...
router.post("/:id/checkin", authMiddleware, async (req, res) => {
    ...
    try {
        ...
        const [habitRows] = await db_1.default.query("SELECT target_count FROM habits WHERE id = ?", [req.params.id]);
        const targetCount = habitRows[0]?.target_count || 1;
        const addMetric = metric_value || 1;
        // check xem đã check-in hôm nay chưa
        const [existing] = await db_1.default.query("SELECT * FROM habit_logs WHERE habit_id = ? AND log_date = ?", [req.params.id, today]);
        ...
        if (existing.length > 0) {
            ...
            await db_1.default.query("UPDATE habit_logs SET metric_value = ?, is_completed = ?, note = ? WHERE id = ?", [newMetric, isCompletedNow ? 1 : 0, note || currentLog.note, currentLog.id]);
            ...
            await db_1.default.query(`INSERT INTO habit_logs (habit_id, user_id, log_date, note, metric_value, metric_unit, is_completed)
         VALUES (?, ?, CAST(? AS DATE), ?, ?, ?, ?)`, [req.params.id, req.user.id, today, note || null, addMetric, metric_unit || null, isCompletedNow ? 1 : 0]);
        ...
        if (isCompletedNow) {
            ...
            const [plantRows] = await db_1.default.query("SELECT experience, level, is_wilted, plant_type FROM plants WHERE user_id = ? ORDER BY id ASC LIMIT 1", [req.user.id]);
            let plantData = null;
...
async function updateHabitStreak(habitId, today) {
    const [habitRows] = await db_1.default.query("SELECT current_streak, last_completed_date FROM habits WHERE id = ?", [habitId]);
    ...
    if (lastDate) {
        ...
        if (diffDays === 0)
            return; // Đã check-in hôm nay rồi
        if (diffDays === 1)
            newStreak += 1; // Liên tiếp
        else
            newStreak = 1; // Bị gián đoạn, reset về 1
    }
    else {
        newStreak = 1; // Lần đầu tiên
    }
    const longestStreak = Math.max(newStreak, habit.longest_streak || 0);
    await db_1.default.query("UPDATE habits SET current_streak = ?, longest_streak = ?, last_completed_date = ? WHERE id = ?", [newStreak, longestStreak, today, habitId]);
}
...
async function updateStreak(userId) {
    ...
    const vietnamTime = new Date(now.getTime() + (7 * 60 * 60 * 1000));
    const today = vietnamTime.toISOString().split('T')[0];
    const [streakRows] = await db_1.default.query("SELECT * FROM streaks WHERE user_id = ?", [userId]);
    if (streakRows.length === 0) {
        await db_1.default.query("INSERT INTO streaks (user_id, current_streak, longest_streak, last_completed_date, freeze_tokens) VALUES (?, 1, 1, ?, 0)", [userId, today]);
        return;
    }
    ...
    await db_1.default.query("UPDATE streaks SET current_streak = ?, longest_streak = ?, last_completed_date = ?, freeze_tokens = ? WHERE user_id = ?", [newStreak, longestStreak, today, freezeTokens, userId]);
}
// ================= DIAGNOSTIC: check reminder + FCM =================
router.get("/check-reminder", authMiddleware, async (req, res) => {
    try {
        const [habitRows] = await db_1.default.query("SELECT id, name, reminder_time, is_active FROM habits WHERE user_id = ? AND is_active = 1 ORDER BY id DESC", [req.user.id]);
        const [userRows] = await db_1.default.query("SELECT id, fcm_token FROM users WHERE id = ?", [req.user.id]);
        const now = new Date();
        const vietnamTime = new Date(now.getTime() + 7 * 60 * 60 * 1000);
        res.json({
            ...
            habits: habitRows,
            habitsWithReminder: habitRows.filter((h) => h.reminder_time != null),
        });
    }
    catch (error) {
        console.error(error);
...
router.post("/test-push", authMiddleware, async (req, res) => {
    try {
        const [userRows] = await db_1.default.query("SELECT fcm_token, name FROM users WHERE id = ?", [req.user.id]);
        const user = userRows[0];
        if (!user || !user.fcm_token) {
            return res.json({ sent: false, reason: "No FCM token", fcm_token: user?.fcm_token || null });
        }
        await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, '🔔 Test thông báo từ Viora', `Xin chào! Đây là thông báo test. Nếu bạn thấy được, FCM đang hoạt động tốt!`, req.user.id);
...
router.post("/send-reminder/:id", authMiddleware, async (req, res) => {
    try {
        const [habits] = await db_1.default.query("SELECT id, name, reminder_time FROM habits WHERE id = ? AND user_id = ?", [req.params.id, req.user.id]);
        const habit = habits[0];
        if (!habit)
            return res.json({ sent: false, reason: "Not found" });
        if (!habit.reminder_time)
            return res.json({ sent: false, reason: "No reminder_time", habit });
        const [userRows] = await db_1.default.query("SELECT fcm_token FROM users WHERE id = ?", [req.user.id]);
        const token = userRows[0]?.fcm_token;
        if (!token)
            return res.json({ sent: false, reason: "No FCM token" });
        await (0, fcm_push_service_1.sendPushNotification)(token, '⏰ Đến giờ rồi!', `Đã đến lúc thực hiện thói quen "${habit.name}" 💪`, req.user.id);
...
router.get("/achievements", authMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT * FROM achievements WHERE user_id = ? ORDER BY unlocked_at DESC", [req.user.id]);
...
async function checkAchievements(userId) {
    ...
    const [existing] = await db_1.default.query("SELECT achievement_key FROM achievements WHERE user_id = ?", [userId]);
    ...
    const [checkinRows] = await db_1.default.query("SELECT COUNT(*) as total FROM habit_logs WHERE user_id = ?", [userId]);
    const totalCheckins = checkinRows[0].total;
    const [habitRows] = await db_1.default.query("SELECT COUNT(*) as total FROM habits WHERE user_id = ? AND is_active = 1", [userId]);
    const totalHabits = habitRows[0].total;
    const [plantRows] = await db_1.default.query("SELECT level FROM plants WHERE user_id = ?", [userId]);
    ...
    for (const ach of ACHIEVEMENTS) {
        if (!unlockedKeys.has(ach.key) && conditions[ach.key]) {
            await db_1.default.query(`INSERT INTO achievements (user_id, achievement_key, title, description, icon)
         VALUES (?, ?, ?, ?, ?)`, [userId, ach.key, ach.title, ach.desc, ach.icon]);
...
// ================= SET PLANT TYPE =================
router.put("/plant/type", authMiddleware, async (req, res) => {
    const { plant_type } = req.body;
    if (!plant_type)
        return res.status(400).json({ message: "Missing plant_type" });
    try {
        const [rows] = await db_1.default.query("SELECT id FROM plants WHERE user_id = ?", [req.user.id]);
        if (rows.length === 0) {
            await db_1.default.query("INSERT INTO plants (user_id, plant_type, level, experience) VALUES (?, ?, 1, 0)", [req.user.id, plant_type]);
        }
        else {
            await db_1.default.query("UPDATE plants SET plant_type = ? WHERE user_id = ?", [plant_type, req.user.id]);
        }
...
// ================= GET PLANT =================
router.get("/plant", authMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query("SELECT * FROM plants WHERE user_id = ?", [req.user.id]);
        if (rows.length === 0) {
            return res.json({
                plant: {
                    plant_type: "sprout",
...
async function updatePlant(userId, today) {
    // Đảm bảo plant tồn tại (chỉ INSERT nếu chưa có)
    const [existingPlants] = await db_1.default.query("SELECT id FROM plants WHERE user_id = ? LIMIT 1", [userId]);
    if (existingPlants.length === 0) {
        await db_1.default.query(`INSERT INTO plants (user_id, plant_type, level, experience, last_watered, health)
       VALUES (?, 'bamboo', 1, 0, ?, 100)`, [userId, today]);
    }
    // Atomic UPDATE: cộng dồn experience
    await db_1.default.query(`UPDATE plants SET experience = experience + 1, last_watered = ? WHERE user_id = ?`, [today, userId]);
    // Đọc lại experience sau khi đã cộng để tính level
    const [plantRows] = await db_1.default.query("SELECT experience, level, is_wilted, plant_type FROM plants WHERE user_id = ? ORDER BY id ASC LIMIT 1", [userId]);
    ...
    if (newLevel !== plant.level) {
        await db_1.default.query(`UPDATE plants SET level = ? WHERE user_id = ?`, [newLevel, userId]);
...
async function recalculatePlantExperience(userId) {
    try {
        console.log(`[Recalculate] Starting for userId=${userId}`);
        // Ensure plant exists
        const [existingPlants] = await db_1.default.query("SELECT id FROM plants WHERE user_id = ? LIMIT 1", [userId]);
        if (existingPlants.length === 0) {
            await db_1.default.query(`INSERT INTO plants (user_id, plant_type, level, experience, health)
         VALUES (?, 'bamboo', 1, 0, 100)`, [userId]);
        }
        // Check if user has any active habits
        const [activeHabits] = await db_1.default.query(`SELECT COUNT(*) as count FROM habits WHERE user_id = ? AND is_active = 1`, [userId]);
        if (activeHabits[0].count === 0) {
            // No active habits, reset plant to initial state
            await db_1.default.query(`UPDATE plants SET experience = 0, level = 1, last_watered = NULL,
        ...
        const [expRows] = await db_1.default.query(`SELECT COUNT(*) as total FROM habit_logs hl
       INNER JOIN habits h ON hl.habit_id = h.id
       WHERE hl.user_id = ? AND h.is_active = 1 AND hl.is_completed = 1`, [userId]);
        const totalExp = expRows[0].total;
        console.log(`[Recalculate] totalExp=${totalExp}`);
        // Get last completed date for last_watered
        const [lastLogRows] = await db_1.default.query(`SELECT hl.log_date FROM habit_logs hl
       INNER JOIN habits h ON hl.habit_id = h.id
       WHERE hl.user_id = ? AND h.is_active = 1 AND hl.is_completed = 1
       ORDER BY hl.log_date DESC LIMIT 1`, [userId]);
        ...
        // Update plant
        await db_1.default.query(`UPDATE plants SET experience = ?, level = ?, last_watered = ? WHERE user_id = ?`, [totalExp, newLevel, lastWatered, userId]);
