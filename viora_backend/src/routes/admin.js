...
const adminMiddleware = async (req, res, next) => {
    try {
        ...
        const [rows] = await db_1.default.query("SELECT role FROM users WHERE id = ?", [userId]);
...
router.put("/users/:userId/role", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        await db_1.default.query("UPDATE users SET role = ? WHERE id = ?", [role, userId]);
...
router.post("/users", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        // Create user
        const [result] = await db_1.default.query("INSERT INTO users (name, email, password, role) VALUES (?, ?, ?, ?)", [name, email, hashedPassword, role || 'user']);
        res.json({
...
router.post("/users/bulk-delete", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        await db_1.default.query(`DELETE FROM users WHERE id IN (${placeholders})`, idsToDelete);
        res.json({
...
router.post("/posts/:postId/report", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { postId } = req.params;
        ...
        const [posts] = await db_1.default.query("SELECT p.*, u.name, u.email, u.language FROM community_posts p JOIN users u ON p.user_id = u.id WHERE p.id = ?", [postId]);
        if (posts.length === 0) {
            return res.status(404).json({ message: "Post not found" });
        }
        const post = posts[0];
        // Mark post as warned
        await db_1.default.query("UPDATE community_posts SET is_warned = 1 WHERE id = ?", [postId]);
...
// ================= UNWARN POST =================
router.put("/posts/:postId/unwarn", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { postId } = req.params;
        const [posts] = await db_1.default.query("SELECT user_id FROM community_posts WHERE id = ?", [postId]);
        if (!posts.length)
            return res.status(404).json({ message: "Post not found" });
        await db_1.default.query("UPDATE community_posts SET is_warned = 0, edited_after_warn = 0 WHERE id = ?", [postId]);
        // Mark old warning notifications as read so they no longer count as unread
        await db_1.default.query("UPDATE user_notifications SET is_read = 1 WHERE type = 'warning' AND JSON_EXTRACT(payload, '$.post_id') = ?", [postId]);
...
router.put("/posts/:postId/approve", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { postId } = req.params;
        const [posts] = await db_1.default.query("SELECT user_id, edited_after_warn FROM community_posts WHERE id = ?", [postId]);
        if (!posts.length)
            return res.status(404).json({ message: "Post not found" });
        await db_1.default.query("UPDATE community_posts SET is_warned = 0, edited_after_warn = 0 WHERE id = ?", [postId]);
        // Mark old warning notifications as read so they no longer count as unread
...
router.delete("/posts/:postId/reject", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { postId } = req.params;
        const { reason } = req.body;
        ...
        const [posts] = await db_1.default.query("SELECT p.user_id, u.name, u.email, u.language FROM community_posts p JOIN users u ON p.user_id = u.id WHERE p.id = ?", [postId]);
        if (!posts.length)
            return res.status(404).json({ message: "Post not found" });
        // Delete post
        await db_1.default.query("DELETE FROM community_posts WHERE id = ?", [postId]);
...
router.get("/comments", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [comments] = await db_1.default.query(`
      SELECT 
...
router.delete("/comments/:commentId", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        await db_1.default.query("DELETE FROM community_comments WHERE id = ?", [commentId]);
...
router.get("/stats", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [userStats] = await db_1.default.query("SELECT COUNT(*) as total_users FROM users");
        const [postStats] = await db_1.default.query("SELECT COUNT(*) as total_posts FROM community_posts");
        const [commentStats] = await db_1.default.query("SELECT COUNT(*) as total_comments FROM community_comments");
        const [todayUsers] = await db_1.default.query("SELECT COUNT(*) as today_users FROM users WHERE DATE(created_at) = CURDATE()");
        const [todayPosts] = await db_1.default.query("SELECT COUNT(*) as today_posts FROM community_posts WHERE DATE(created_at) = CURDATE()");
        const [habitStats] = await db_1.default.query("SELECT COUNT(*) as total_habits FROM habits WHERE is_active = 1");
        const [activeUsers] = await db_1.default.query("SELECT COUNT(DISTINCT user_id) as active_users FROM habit_logs WHERE log_date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)");
        res.json({
...
router.get("/growth", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const [userGrowth] = await db_1.default.query(`
      SELECT 
    ...
    `, [days]);
        ...
        const [postGrowth] = await db_1.default.query(`
      SELECT 
        DATE(created_at) as date,
        COUNT(*) as daily_count
    ...
    `, [days]);
        ...
        if (userGrowth.length === 0) {
            const [allTimeUsers] = await db_1.default.query(`
        SELECT 
          DATE(created_at) as date,
          COUNT(*) as daily_count
        FROM users
      ...
      `, [days]);
        ...
        if (postGrowth.length === 0) {
            const [allTimePosts] = await db_1.default.query(`
        SELECT 
          DATE(created_at) as date,
          COUNT(*) as daily_count
        FROM community_posts
      ...
      `, [days]);
        ...
        const fillMissingDates = (data, daysCount = days) => {
            const result = [];
        ...
        const formattedUserGrowth = filledUserGrowth.map((row) => {
        ...
        const formattedPostGrowth = filledPostGrowth.map((row) => {
...
router.get("/habits", authMiddleware, adminMiddleware, async (req, res) => {
    try {
    ...
    `;
        ...
        const [habits] = await db_1.default.query(sql, params);
        res.json({ habits });
    }
    catch (error) {
...
router.get("/habits/categories", authMiddleware, adminMiddleware, async (req, res) => {
    try {
    ...
    `);
        const [total] = await db_1.default.query("SELECT COUNT(*) as total FROM habits WHERE is_active = 1");
...
router.get("/habits/trends", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const period = req.query.period || 'weekly';
        const days = period === 'weekly' ? 7 : 30;
        // Daily completion counts
        const [dailyCompletions] = await db_1.default.query(`
      SELECT 
    ...
    `, [days]);
        ...
        const [categoryBreakdown] = await db_1.default.query(`
      SELECT 
        h.category,
    ...
    `, [days]);
        ...
        const [topHabits] = await db_1.default.query(`
      SELECT 
...
router.get("/plants/:userId/history", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { userId } = req.params;
        // First get user by their primary key (id)
        const [userRows] = await db_1.default.query("SELECT id, name, email, avatar_url, created_at FROM users WHERE id = ?", [userId]);
        ...
        let [plantRows] = await db_1.default.query("SELECT * FROM plants WHERE user_id = ? ORDER BY id ASC LIMIT 1", [user.id]);
        // If no plant, create one based on habit_logs data
        if (plantRows.length === 0) {
            const [expRows] = await db_1.default.query(`SELECT COUNT(*) as total_exp,
                MAX(hl.log_date) as last_date
         FROM habit_logs hl
         JOIN habits h ON hl.habit_id = h.id AND h.is_active = 1
         WHERE hl.user_id = ? AND hl.is_completed = 1`, [user.id]);
            ...
            await db_1.default.query(`INSERT INTO plants (user_id, plant_type, level, experience, health, last_watered, days_without_checkin)
         VALUES (?, 'bamboo', ?, ?, 100, ?, 0)`, [user.id, level, totalExp, lastDate]);
            // Re-query after insert
            [plantRows] = await db_1.default.query("SELECT * FROM plants WHERE user_id = ? ORDER BY id ASC LIMIT 1", [user.id]);
        }
        if (plantRows.length === 0) {
        ...
        let calcLevel = 1;
        for (let i = thresholds.length - 1; i >= 0; i--) {
            if (plant.experience >= thresholds[i]) {
                calcLevel = i + 1;
                break;
            }
        }
        ...
        // Get current streak
        const [streaks] = await db_1.default.query("SELECT current_streak, longest_streak FROM streaks WHERE user_id = ?", [userId]);
        const streak = streaks.length > 0 ? streaks[0] : { current_streak: 0, longest_streak: 0 };
        // Get total habits completed
        const [habitCount] = await db_1.default.query("SELECT COUNT(DISTINCT DATE(log_date)) as days_completed, COUNT(*) as total_habits FROM habit_logs WHERE user_id = ?", [userId]);
        ...
        const [history] = await db_1.default.query(`
      SELECT 
        hl.log_date,
        hl.habit_id,
...
router.get("/auto-reminder/settings", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [settings] = await db_1.default.query("SELECT * FROM auto_reminder_settings WHERE id = 1");
        if (settings.length === 0) {
            // Create default settings if not exists
            await db_1.default.query("INSERT INTO auto_reminder_settings (id, is_enabled, morning_time, evening_time) VALUES (1, 0, '08:00:00', '20:00:00')");
            return res.json({
                is_enabled: false,
...
router.put("/auto-reminder/settings", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { is_enabled, morning_time, evening_time, send_morning, send_evening } = req.body;
        await db_1.default.query(`UPDATE auto_reminder_settings 
       SET is_enabled = ?, morning_time = ?, evening_time = ?, send_morning = ?, send_evening = ?
       WHERE id = 1`, [is_enabled ? 1 : 0, morning_time, evening_time, send_morning ? 1 : 0, send_evening ? 1 : 0]);
...
router.get("/auto-reminder/messages", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [messages] = await db_1.default.query("SELECT * FROM auto_reminder_messages ORDER BY created_at DESC");
        res.json({ messages });
    }
    catch (error) {
...
router.post("/auto-reminder/messages", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { message } = req.body;
        if (!message || message.trim().length === 0) {
            return res.status(400).json({ message: "Message is required" });
        }
        const [result] = await db_1.default.query("INSERT INTO auto_reminder_messages (message, is_active) VALUES (?, 1)", [message.trim()]);
        res.json({
...
router.put("/auto-reminder/messages/:id", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        const { message, is_active } = req.body;
        await db_1.default.query("UPDATE auto_reminder_messages SET message = ?, is_active = ? WHERE id = ?", [message, is_active ? 1 : 0, id]);
...
router.delete("/auto-reminder/messages/:id", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { id } = req.params;
        await db_1.default.query("DELETE FROM auto_reminder_messages WHERE id = ?", [id]);
...
// ================= APP SETTINGS =================
// Get app settings
router.get("/app-settings", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [settings] = await db_1.default.query("SELECT * FROM app_settings WHERE setting_key IN ('app_name', 'app_logo_url')");
        const settingsObj = {};
...
// Update app name
router.put("/app-settings/name", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { appName } = req.body;
        if (!appName || appName.trim().length === 0) {
            return res.status(400).json({ message: "App name is required" });
        }
        await db_1.default.query(`INSERT INTO app_settings (setting_key, setting_value) 
       VALUES ('app_name', ?) 
       ON DUPLICATE KEY UPDATE setting_value = ?`, [appName.trim(), appName.trim()]);
...
// Update app logo
router.put("/app-settings/logo", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const { logoUrl } = req.body;
        await db_1.default.query(`INSERT INTO app_settings (setting_key, setting_value) 
       VALUES ('app_logo_url', ?) 
       ON DUPLICATE KEY UPDATE setting_value = ?`, [logoUrl || null, logoUrl || null]);
...
router.get("/reports", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        const [rows] = await db_1.default.query(`SELECT n.id, n.user_id AS admin_id, n.type, n.title, n.body, n.emoji, n.payload, n.is_read, n.created_at,
       ...
       ORDER BY n.created_at DESC`);
        const reports = await Promise.all(rows.map(async (row) => {
            ...
            if (payload?.post_id) {
                const [posts] = await db_1.default.query("SELECT id, content, image_url, created_at, user_id FROM community_posts WHERE id = ?", [payload.post_id]);
                if (posts.length > 0) {
                    const [authors] = await db_1.default.query("SELECT id, name, email FROM users WHERE id = ?", [posts[0].user_id]);
                    postInfo = {
                        id: posts[0].id,
            ...
            if (payload?.reporter_id) {
                const [reps] = await db_1.default.query("SELECT id, name, email FROM users WHERE id = ?", [payload.reporter_id]);
                if (reps.length > 0) {
        ...
        // Filter only pending
        const pending = reports.filter((r) => r.payload?.status === 'pending');
...
router.put("/reports/:notifId/handle", authMiddleware, adminMiddleware, async (req, res) => {
    try {
        ...
        const [notifs] = await db_1.default.query("SELECT * FROM user_notifications WHERE id = ? AND type = 'post_reported'", [notifId]);
        if (notifs.length === 0) {
        ...
        if (action === 'warn') {
           ...
           VALUES (?, ?, ?, ?, ?, ?, 0, NOW())`, [
            ...
            await db_1.default.query("UPDATE user_notifications SET payload = ? WHERE id = ?", [JSON.stringify(payload), notifId]);
