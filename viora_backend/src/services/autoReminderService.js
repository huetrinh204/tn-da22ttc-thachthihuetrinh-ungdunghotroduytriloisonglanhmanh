...
async function sendAutoReminders(timeOfDay) {
    try {
        ...
        const [messages] = await db_1.default.query("SELECT * FROM auto_reminder_messages WHERE is_active = 1");
            ...
            try {
                ...
                */
                ...
                if (user.fcm_token && user.fcm_token.trim() !== '') {
                    try {
                        await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, '⏰ Nhắc nhở từ Viora', messageText);
