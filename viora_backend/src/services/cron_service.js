...
exports.sendMorningEmails = sendMorningEmails;
exports.sendEveningEmails = sendEveningEmails;
const node_cron_1 = __importDefault(require("node-cron"));
const db_1 = __importDefault(require("../config/db"));
const email_service_1 = require("./email_service");
const fcm_push_service_1 = require("./fcm_push_service");
const autoReminderService_1 = require("./autoReminderService");
...
async function checkAndSendAutoReminders() {
    try {
        ...
        if (setting.send_morning === 1 && setting.morning_time) {
            const [mHour, mMinute] = setting.morning_time.split(':').map((n) => parseInt(n));
            if (currentHour === mHour && currentMinute === mMinute) {
                console.log('[Cron] Triggering morning auto reminder');
                await (0, autoReminderService_1.sendAutoReminders)('morning');
            }
        }
        // Parse evening time
        if (setting.send_evening === 1 && setting.evening_time) {
            const [eHour, eMinute] = setting.evening_time.split(':').map((n) => parseInt(n));
            if (currentHour === eHour && currentMinute === eMinute) {
                console.log('[Cron] Triggering evening auto reminder');
                await (0, autoReminderService_1.sendAutoReminders)('evening');
            }
        }
    }
    catch (error) {
...
async function checkAndSendPersonalReminders() {
    try {
        ...
        const [users] = await db_1.default.query(`SELECT id, name, fcm_token, 
       ...
       WHERE fcm_token IS NOT NULL AND fcm_token != '' AND (
        ...
        for (const user of users) {
            ...
            if (isMorning) {
                await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "🌱 Chào buổi sáng!", `Xin chào ${user.name}! Hôm nay bạn đã sẵn sàng cho thói quen chưa?`);
            }
            else {
                // Evening — chỉ gửi nếu chưa hoàn thành hết
                const [habitRows] = await db_1.default.query("SELECT COUNT(*) as total FROM habits WHERE user_id = ? AND is_active = 1", [user.id]);
                const total = habitRows[0].total;
                if (total === 0)
                    continue;
                const [doneRows] = await db_1.default.query(`SELECT COUNT(*) as done FROM habit_logs hl
           INNER JOIN habits h ON hl.habit_id = h.id AND h.is_active = 1
           WHERE hl.user_id = ? AND hl.log_date = ?`, [user.id, today]);
                const done = doneRows[0].done;
                if (done < total) {
                    await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "🌙 Nhắc nhở buổi tối", `Bạn còn ${total - done}/${total} thói quen chưa hoàn thành hôm nay!`);
                }
            }
        }
    }
...
async function sendMorningEmails() {
    try {
        const [users] = await db_1.default.query("SELECT id, name, email, fcm_token FROM users WHERE email IS NOT NULL AND email != ''");
        for (const user of users) {
            // Gửi email
            await (0, email_service_1.sendMorningReminder)(user.email, user.name);
    ...
    try {
        ...
        for (const user of users) {
         ...
         WHERE hl.user_id = ? AND hl.log_date = ?`, [user.id, today]);
            ...
            if (done < total) {
                // Gửi email
                await (0, email_service_1.sendEveningReminder)(user.email, user.name, done, total);
                // Gửi FCM push nếu có token
                if (user.fcm_token) {
                    await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "🌙 Nhắc nhở buổi tối", `Bạn còn ${total - done}/${total} thói quen chưa hoàn thành hôm nay!`);
                }
...
async function checkAndSendHabitReminders() {
    try {
        ...
        const [habits] = await db_1.default.query(`SELECT h.id as habit_id, h.name as habit_name, h.reminder_time,
       ...
       WHERE h.reminder_time IS NOT NULL
        ...
        for (const habit of habits) {
            ...
            if (diffMin < 0 || diffMin > 60)
                continue;
            // TODO: Đổi lại thành % 20 sau khi test xong
            // Test: gửi mỗi 2 phút (0, 2, 4, 6, ...) để kiểm tra cron
            if (diffMin % 2 !== 0)
                continue;
            await (0, fcm_push_service_1.sendPushNotification)(habit.fcm_token, '⏰ Đến giờ rồi!', `Đã đến lúc thực hiện thói quen "${habit.habit_name}" 💪`, habit.user_id);
...
async function checkProgressiveWilting() {
    ...
    `);
        for (const user of users) {
            ...
            const [logs] = await db_1.default.query(`SELECT COUNT(*) as count 
         FROM habit_logs hl
         INNER JOIN habits h ON hl.habit_id = h.id AND h.is_active = 1
         WHERE hl.user_id = ? AND hl.log_date = ?`, [user.user_id, yesterday]);
            const completedYesterday = logs[0].count > 0;
            if (completedYesterday) {
                // User was active yesterday - reset counter
                if (user.days_without_checkin > 0) {
                    await db_1.default.query(`UPDATE plants 
             SET days_without_checkin = 0, 
                 is_wilted = 0,
                 last_penalty_date = NULL
             WHERE id = ?`, [user.plant_id]);
                ...
                await db_1.default.query(`UPDATE plants SET days_without_checkin = ? WHERE id = ?`, [newDays, user.plant_id]);
                ...
                if (newDays >= 3) {
                    ...
                    if (user.freeze_tokens > 0) {
                        ...
                        await db_1.default.query("UPDATE streaks SET freeze_tokens = ? WHERE user_id = ?", [newTokens, user.user_id]);
                        await db_1.default.query("UPDATE plants SET days_without_checkin = 0, is_wilted = 0 WHERE id = ?", [user.plant_id]);
                        console.log(`[Wilting] User ${user.name} - Freeze token used! Tokens left: ${newTokens}`);
                        if (user.fcm_token) {
                            await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "🧊 Streak Freeze đã được dùng!", `Bạn đã bỏ lỡ hôm qua nhưng streak được bảo vệ. Còn ${newTokens} freeze token. Hãy check-in hôm nay! 💪`, user.user_id);
                        }
                    }
                    else {
                 ...
                 SET is_wilted = 1, 
                     experience = ?,
                     last_penalty_date = ?
                 WHERE id = ?`, [newExp, today, user.plant_id]);
                            console.log(`[Wilting] User ${user.name} - Plant wilted! Applied -${penalty} EXP penalty`);
                            if (user.fcm_token) {
                                await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "🍂 Cây của bạn đang héo!", `Cây đã bị mất ${penalty} điểm vì không check-in ${newDays} ngày. Hãy quay lại ngay! 💧`, user.user_id);
                            }
                        }
                    }
                }
                    ...
                    if (user.fcm_token) {
                        await (0, fcm_push_service_1.sendPushNotification)(user.fcm_token, "⚠️ Cây cần được chăm sóc!", "Cây sẽ bị héo và mất điểm nếu bạn không check-in trong 24 giờ tới!", user.user_id);
                    }
                }
            }
        }
